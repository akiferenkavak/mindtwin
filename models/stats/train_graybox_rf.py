...
def _detect_axis_act_col(headers) -> str | None:
    norm = {k.lower(): k for k in headers if isinstance(k, str)}
    return norm.get("axis_act")


def _detect_brake_sig_col(headers) -> str | None:
    norm = {k.lower(): k for k in headers if isinstance(k, str)}
    return norm.get("brake_sig")


def _detect_vel_axis_act_cols(headers) -> list | None:
...
def _detect_curr_cols(headers) -> list | None:
...
def _parse_axis_act(raw: str) -> list[float] | None:
